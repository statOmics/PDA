...
centers all samples (columns) so that they all match the grand median by subtracting the respective columns medians differences to the grand median (code in script: `qf <- normalize(qf,   i = "precursors_log", name = "precursors_norm", method = "diff.median")`.  What do you observe and try to explain this.
...
Note, that the data are from data dependent acquistion and searched with maxQuant data, so the data processing will be slightly different. 
