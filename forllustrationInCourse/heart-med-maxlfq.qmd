...
pe <- sweep( #Subtract log2 norm factor column-by-column (MARGIN = 2)
  pe,
  MARGIN = 2,
  STATS = nfLogMedian(pe,i = "peptides_log",na.rm = TRUE),
  i = "peptides_log",
  name = "peptides_norm"
)
```
...
```{r brixenHeartVolcanoAVG}
 inferences |> filter(contrast == "tissueV + 0.5 * locationR:tissueV") |> plotVolcano() + ggtitle("Default median normalisation")
```
