...
We can reproduce the results ([Vanderaa and Gatto 2022](https://uclouvain-cbio.github.io/SCP.replication/articles/brunner2022.html)): 
...
- In-house DIA spike-in study to illustrate the analysis (subset of [@Staes2024]). 
...
```{r echo=FALSE}
knitr::include_graphics("./figs/brixenNormMedian.jpg")
```
...
- 3 patients
- proteomes from biopsies of atrium and ventriculum in the left
and the right heart region 
- DDA

```{r echo=FALSE}
knitr::include_graphics("./figs/brixenHeartNormalisationEffect.jpg")
```
...
- Also correct for differences in 
...
- Problematic
- Biased FC
$\rightarrow$ Does not account for precursor species. Missingness.
...
$$
t=\frac{\log_2\widehat{\text{FC}}}{\text{se}_{\log_2\widehat{\text{FC}}}}=\frac{`r format(summary(lm1)$coef[2,1], digit = 3)`}{`r format(summary(lm1)$coef[2,2], digit = 3)`}=`r format(summary(lm1)$coef[2,3], digit = 3)`
$$

- Is t = `r format(summary(lm1)$coef[2,3], digit = 3)` indicating that
there is an effect?

- How likely is it to observe
t = `r format(summary(lm1)$coef[2,3], digit = 3)` when there is no effect of the argP KO on the protein expression?
...
- When we repeat the experiment, the probability to observe a fold change for this protein that is more extreme than a `r format(2^abs(lm1$coef[2]),digits=3)` fold ($\log_2 FC=`r format(lm1$coef[2],digits=3)`$) down or up regulation by random change (if $H_0$ is true) is `r round(summary(lm1)$coef[2,4]*1e4,0)` out of 10000.  
...
- So an upperbound of the expected FP is $m \times \alpha$ or $1020 \times 0.05=`r round(1020*0.05,0)`$. 
...
$$
   T_p^{mod} = \frac{\log_2 \widehat{\text{FC}}
}{C \quad \tilde{S}_p} ,
 $$
